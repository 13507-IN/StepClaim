  ...
  return (
    <div className="space-y-12 pt-8 pb-12">
      ...
      <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
        <div>
          <h2 className="text-4xl font-black text-white leading-tight tracking-wider font-[family-name:var(--font-header)] uppercase">
            Overview
          </h2>
          <p className="mt-3 max-w-2xl text-base leading-relaxed text-slate-300">
            Welcome back,{" "}
            <span className="font-bold text-white">{user?.username}</span>.
            Check your progress and ready your gear.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            size="sm"
            variant="outline"
            className="h-10 border-white/10 bg-white/5 px-4 text-sm font-semibold text-slate-100 hover:bg-white/10"
          >
          ...
          <Button
            asChild
            size="sm"
            className="h-10 bg-linear-to-r from-cyan-500 to-purple-600 px-4 text-sm font-bold tracking-wide"
          >
            <Link href="/run" className="flex items-center gap-1.5">
      ...
      <div className="space-y-6 pt-4">
        <h3 className="text-2xl font-black text-white leading-tight tracking-wider font-[family-name:var(--font-header)] uppercase">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {data.runs.slice(0, 5).map((run: any, idx: number) => {
              ...
              return (
                <ActivityItem key={run.id} activity={act} delay={idx * 0.05} />
              );
            })}
          </div>
        )}
